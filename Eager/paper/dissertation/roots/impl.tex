...
identify SLO-violating performance anomalies.  For anomalies not caused by workload
changes (HTTP request rate), Roots performs further analysis to identify the
...
Because AppScale is open source, we were able to modify its implementation minimally to integrate 
Roots. 
...
When enabled for a given application, the SLO-based anomaly detector starts a benchmarking process
...
imposes a limit on the number of samples to keep in memory.
\end{itemize}

Once the anomaly detector identifies an SLO violation, it will continue to 
detect the same violation
...
for the bottleneck. These candidates are then further evaluated by a weighting algorithm to
...
We take all these $T_{total}$ values
...
large relative to $T_{X_n}$.  Often these rare events are correlated with large
...
event in the PaaS software).  Overall, Roots detects these events, and identifies them correctly (as
explained below), but they
...
The component associated with the highest ranked regressor (i.e. highest relative importance) is chosen as a bottleneck candidate.
...
relative importance of $r$ within each segment. This way we
