...
or SLO/SLA violations~\cite{6045942}. 
...
software stack, and it does not require application-level instrumentation.
...
that detail the key features of cloud APMs. Consequently, we strive to incorporate requirements like
scalability, autonomy and dynamic resource management into our design.
...
virtualization layer to system resource bottlenecks. Our plan for Roots is highly in sync with this
vision. We currently support identifying bottlenecks from a set of kernel services provided by the PaaS cloud.
...
We use application response time (latency) as an indicator of anomalies,
...
%It is indeed possible to build an anomaly detector that trains a model
