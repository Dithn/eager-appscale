...
\begin{enumerate}
\item the cloud-hosted applications are implemented and deployed according to a set of
developer and organizational standards, and
\item they operate as expected while maintaining a certain quality-of-service level.
\end{enumerate}

This is a broad definition that allows room for many potential avenues of research.
In our research we explore three of them. We briefly describe the research objectives
that constitute each avenue here, and provide detailed discussions in the chapters that
follow.

First, we wish to ensure that all applications deployed in a given cloud platform follow a set of
development and deployment policies that are specified by a platform or organizational administrator.
These policies may include specific
dependency management practices, naming and packaging standards for software artifacts, 
software versioning requirements, and practices that enable software artifacts to evolve maintaining backward compatibility. 
Many of the conditions that constitute these policies are the result of decades of software engineering research and practice. 
However, each user and organization may wish to follow some variation of them. Therefore
cloud platforms should allow administrators to specify custom
policies that fit their use cases. Once specified, the cloud platform
should enforce the policies in a scalable manner, without causing the application performance to
deteriorate. The cloud platform should also ensure that the applications maintain policy compliance at
all times -- even through system and application updates.

Secondly, we look at formulating concrete service level objectives (SLOs) and 
service level agreements (SLAs) concerning the performance of cloud-hosted applications.
An SLO specifies a system's minimum quality of service (QoS) level in a measurable and
controllable manner. They may cover various QoS
...
In practice, SLOs are used to form an SLA --
...
penalties for failing to maintain those QoS levels. Such agreements are crucial for
mission critical applications, interactive applications and most web and mobile applications.
However, today's cloud platforms lack the ability to reason about application performance.
Consequently, existing cloud platforms only offer SLAs regarding service availability (uptime)
to their users. Therefore
cloud platforms should be enhanced with mechanisms to analyze and reason about 
application performance limits, so they can offer performance SLOs and SLAs.

Finally, we wish to implement performance monitoring and diagnosis for cloud-hosted applications.
In order to detect violations of performance SLOs, and violations of policies that govern the performance aspects of
applications, cloud platforms should continuously monitor application performance.
Cloud platforms should be able to detect problems in a timely manner before
they escalate into major outages or SLA violations. For each detected problem, the cloud
platform should also be able to diagnose the potential root cause, thereby enabling faster
remediation.
...
consistently meeting performance expectations (SLOs) of the developers and the users. We strive to design
...
over the applications and related components residing in the cloud. The goal of our research is to provide governance
