...
\item [Performance service level objectives]
...
to detect SLO violations and other performance anomalies. It also includes diagnosing the root
cause of each detected anomaly, so issues can be resolved faster. 
This typically requires collecting run-time data from many different
components in the cloud platform, and correlating them to identify patterns and trends. 
\end{description}

These governance features are crucial for
achieving administrative conformance, developer best practices,
and performance objectives. But none of these features are implemented satisfactorily in the
...
often combines local program logic with calls to remote APIs. 
...
This proliferation of web APIs in the cloud demands that governance
