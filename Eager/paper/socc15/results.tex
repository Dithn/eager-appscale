...
the values in the observation history.  When ever a new observation is
...
That is, we keept rest of Cerebro unchanged, swaped QBETS out for the simple
predictor, and ran the same set of experiments using the logged observations. 
%Then we make
%$95^{th}$ percentile
...
Thus the results in Figure~\ref{fig:simple_pred_accuracy} are directly comparable to
Figure~\ref{fig:accuracy_summary} where Cerebro used QBETS as a forecaster.  
...
For the simple prdictor, Figure~\ref{fig:simple_pred_accuracy} shows lower 
...
and QBETS forecasts for the Rooms\#getRoomsInCity operation on Google App
...
the correctness percentage is $97.3\%$ (c.f. Figure~\ref{fig:accuracy_summary}) compared to
$75.1\%$ for the simple predictor (from
Figure~\ref{fig:simple_pred_accuracy}).
