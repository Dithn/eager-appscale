...
\subsection{Effectiveness and Learning Duration of QBETS}
\label{sec:learning}

In order to gauge the effectiveness of QBETS, we implement
another SLA predictor, which we refer to as the \textit{simple predictor}. This implementation
predicts the $p$-th percentile of a time series by simply calculating the actual $p$-th percentile of the
time series in the conventional manner. 
We keep rest of Cerebro unchanged, and swap QBETS out for the simple predictor. We have
implemented our prototype in a modular fashion that makes such alterations trivial. Then we make
$95^{th}$ percentile
...
reuse the same cloud SDK monitoring data and application benchmarking data that was used in Subsection~\ref{sec:correctness}. Therefore results in Figure~\ref{fig:simple_pred_accuracy} are directly comparable to
Figure~\ref{fig:accuracy_summary}.  

\begin{figure}
\centering
\includegraphics[scale=0.35]{simple_pred_accuracy_summary}
\caption{Cerebro correctness percentage in Google App Engine and AppScale cloud platforms, resulting from
the simple predictor (without QBETS).}
\label{fig:simple_pred_accuracy}
\vspace{-0.2in}
\end{figure}

Figure~\ref{fig:simple_pred_accuracy} shows lower percentages compared to Figure~\ref{fig:accuracy_summary}.
Also in several cases, the simple predictor falls well short of the desired correctness goal of $95\%$.
...
To understand why the simple predictor fails to meet the desired accuracy level, we take the
Rooms\#getRoomsInCity operation on Google App Engine (the case that shows lowest accuracy level), 
and compare its predicted SLAs
and actual response times within a period of 1000 minutes. The resulting plot is shown in
Figure~\ref{fig:get_rooms_in_city}. According to this, there is a significant number of response
time measurements that violate the SLA given by simple predictor, but are below the QBETS
predicted SLA. Thus, the extra conservativeness of QBETS enables it to predict more accurate SLAs.
...
generates predictions while consistently maintaining its accuracy level above 95\%. But other
approaches such as the simple predictor fails to meet this goal even after 1000 minutes has elapsed.
