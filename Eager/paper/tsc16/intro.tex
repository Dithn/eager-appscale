...
and latency, Roots also records the internal PaaS services invoked by applications,
...
runtime data collected by Roots. This mechanism can also calculate the proportion of 
user requests processed by each path, which can be used to characterize the workload
of an application (e.g. read-heavy vs write-heavy workload in a data management
application). Based on that, Roots can monitor for characteristic changes in the application
workload.
