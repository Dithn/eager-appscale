\textcolor{blue}{It seems like there are four things we want to test to
...
derived from steady-state workload and you don't change the workload).  Point
(3) you have covered thoroughly in the case where there hasn't been a workload
change.  However in (2) or (3) you haven't shown how well Roots works when
there is a workload change.  Discussion of (4) is implied in the text.}

\textcolor{blue}{I also suggested a summarization at the end of the efficacy
subsection (see comments below).  What you could do here is to put in result
showing that the bottleneck detector does not erroneously identify bottlenecks
when the workload changes and then simply cover these four issues in the
summarization.} 
 
\textcolor{green}{We wish to understand both the efficacy of the Roots system
in terms of its ability to identify performance anomalies and to correctly
determine the component responsible for each anomaly.  Additionally, we wish
to characterize ovrehead introduced by Roots in terms of its effect on
perceived application performance and to determine the degree to which Roots
...
%We first evaluate the effectiveness of Roots as an anomaly detection mechanism. 
\textcolor{green}{To begin the evaluation of the Roots prototype,}
we experiment with
the SLO-based anomaly detector, using a simple HTML-producing web application
called ``guestbook''~\cite{XXX}.
This application allows users to login, and post comments. It uses the
\textcolor{green}{GAE}  datastore service to save
the posted comments, and the \textcolor{green}{GAE} user management service to handle authentication. Each request processed
...
the SLO \textcolor{green}{success} probability at 95\%, and set the response time upper bound to $\mu_g + n\sigma_g$. 
...
\textcolor{green}{to cause the datastore service to be slow to respond.}
%. We run a set of experiments
%by injecting faults into the datastore service. 
...
\textcolor{green}{We chose 45ms because it is equal 
to $\mu_g + 5\sigma_g$ for the AppScale deployment under test.} Therefore this delay is sufficient to violate all three SLOs used in our experiments. 
...
\textcolor{green}{(i.e. one
that we did not explicitly inject but none the less caused an SLO violation)} at 19:52 hours
...
caching to the read operation using the AppScale's memcache~\cite{XXX} service. The application contains
...
the guestbook application \textcolor{green}{and follow with}
%. Later we will show some 
...
\textcolor{green}{including one
occured in AppScale itself and
that we did not inject explicitly}.
...
query has frequently taken a long time to execute \textcolor{green}{(again, on
