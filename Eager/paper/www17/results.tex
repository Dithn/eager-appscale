...
in the system.
\textcolor{blue}{You need some discussion of whether these additional
non-injected faults are ``false positives,'' ``natural anomalies,'' or actual
``performance faults'' that caused an SLO violation.}
...
chance. \textcolor{blue}{Again, are these false positives or actual SLO
violations that you simply did not inject?}  These also resulted in two anomaly detection events as shown in the figure.
...
\textcolor{blue}{Here is a discussion of the ``false positive'' issue.
Perhaps move this up to just after the discussion of
figures~\ref{fig:time_line_guestbook_2s} and
~\ref{fig:time_line_guestbook_2s_user} to make this clearer.} 
Now we evaluate the bottleneck identification capability of Roots. In the experimental run illustrated in 
...
\textcolor{blue}{First, I don't think it was clear that there are only two
PaaS kernel invocations in the previous experiments.  You should emphasize
that when you present the initial experiments (which are easier to analyze)
and then say that you'll show
results for a more complex application later.}
...
in this test, Roots correctly selected the first datastore call in the
application code as the bottleneck. \textcolor{blue}{But it also looks like
there are a couple of non-injected detections.  Is this true?  Were they true
violations and did we detect the cause?}
