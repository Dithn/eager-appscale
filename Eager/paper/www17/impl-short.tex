...
The Roots SLO anomaly detector
...
%Whenever it detects that the application has failed to meet the SLO, it triggers an anomaly event. 
...
%Note that this is different from the path distribution analyzer,
...
%We use the ``number of requests
%per unit time'' as the metric of workload size. 
%This information can be obtained from the Roots
%data storage as a time series. 
...
%The Roots bottleneck identification algorithm uses a combination
...
%find that it produces accurate results nearly 100\% of the time. 
%It has been 
...
%Note that for each application request
