...
\textit{blue}{From a generality perspective, detectors and handlers aren't
enough.  You also need rules that trigger actions
(or do not trigger actions) based on a sequence of
detectors and handler results.  
For example, in your current implementation of roots, when 
you detect an anaomaly in the response time and also an analomaly 
in the workload, you trigger a root cause identification analysis.  There
needs to be some program that expresse this conditional relationship as well.
Probably need to say that the Roots programmer needs to write this program
explicitly.}
...
using an internal communication bus, so that events triggered by one anymaly
...
\subsubsection{Appropriateness for Cloud Environments}

\textcolor{blue}{This argument for Roots probably belongs in the introduction.
Also, the lead bullet is scalability.  The reader thus expects some set of
scalability experiments to be in the results section.}

Roots architecture is heavily inspired by previous work that highlight the key features of cloud
monitoring frameworks~\cite{DaCunhaRodrigues:2016:MCC:2851613.2851619,Ibidunmoye:2015:PAD:2808687.2791120}.
