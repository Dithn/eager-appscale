...
\textcolor{green}{However, this wide-spread adoption of cloud computing, particularly for
web services, has been facilitated by ever-deepening software abstractions
making the diagnosis of performance problems challenging.  As
abstractions elide more of the complexity necessary to enable scale,
they also obscure the true causes of performance faults and anomalies.
Thus the rapid growth in the popularity of cloud technologies
...
Specifically,
application developers and users as well as cloud administrators wish to monitor application performance, detect 
anomalies, and identify system bottlenecks. To obtain this level of deep operational insight into
cloud-hosted applications, the cloud platforms must support 
...
today either do not provide any application monitoring support, or rely on the
programmers themselves to introduce application-level logging and
instrumentation.  As a result, cloud administrators faced with applications
that are failing to meet their performance targets must that the performance
instrumentation is both correct and not exacerbating the 
problems at hand.  Further, because the programmer is often relying on extant
installed services (e.g. scalable database services, network load balancing,
scalable in-memory caching services, etc.) that are performance opaque, it is
often difficult or impossible to diagnose the ``root cause'' of a problem
within the application or cloud software infrastructure.}
...
the events that generate the data at
different layers and in different components. 
%
...
length of the history or sample size to consider when performing statistical analysis, and the performance 
\textcolor{green}{SLOs (service level objectives~\cite{XXX}) that are associated with SLAs
(service level agreements) that may govern the application.}
%to maintain over time and check for
%violations. 
Cloud monitoring
should be able to facilitate these diverse requirements \textcolor{green}{on a
...
monitor (APM) called \textit{Roots} that can be \textcolor{green}{integrated
with} variety of cloud Platform-as-a-Service (PaaS) technologies. 
...
\textcolor{green}{To be able to correlate application activity with system
events,} 
...
\textcolor{green}{Thus we summarize the contributions made by this paper as
follows:
\begin{itemize}
\item we describe the architecture of Roots as a primordial intrinsic PaaS
service (i.e. on that does not require or depend upon application
instrumentation and work automatically),
\item we describe a statistical methodology for determining when an
application is experiencing a performance fault (as opposed to an increase in
workload) and identifying
the layer or
application component that is responsible for the fault, and
