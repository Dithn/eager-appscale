...
bottlenexk component when a violation is not induced by a workload change.
Finally, we wish to investigate the performance and scalability fo the Roots
prototype. 
%\begin{itemize}
...
\caption{SLO-violation Anomaly detection in guestbook application (timeline).}
...
\caption{SLO-violation Anomaly detection in stock-trader application (timeline).}
...
\caption{SLO-violation Anomaly detection in 8 applications deployed in a clustered AppScale cloud.}
...
Finally, to investigate how Roots scalability, we deploy
...
%
With 100 concurrent detectors, the memory usage was 200MB and the CPU usage
50\% of a single core.  Using just the one machine, with 4 cores and 4GB of
memory, a single pod was able to service 40000 concurrent detectors. 
Since one pod can service several applications simultaneously
 and pods are independent,
these results indicate that it is possible to scale Roots effectively.
