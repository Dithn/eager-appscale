...
In addition to submitting application deployment requests to EAGER, the
EAGER developer tools are also responsible for generating API specifications for the
web APIs embodied in the application. For this purpose, the developer tools
should be able to perform some form of source code analysis on the application, and
generate API specifications in a suitable API specification language. Generated specifications
are also sent to EAGER as a part of the application deployment request. If more control is
required over how these API specifications are generated, the EAGER developer tools
can be offered as a trusted service hosted in the cloud. Developers will submit their source code
