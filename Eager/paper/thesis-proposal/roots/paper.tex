...
will support binary segmentation, PELT method and Chen and Lui algorithm. We also wish
...
availability and faults. We studied a number of such frameworks including Nagios,
OpenNMS, Shinken and Zabbix. While all of them support data collection, storage,
...
as New Relic, Datadog and Dynatrace provide hosted solutions with
...
processing systems, and some of them even facilitate monitoring
all the way down to code fragments and SQL queries in user applications.
But since these are third party monitoring frameworks, they cannot
monitor any parameters that are not exposed by the
cloud providers. Therefore they are not able to trace request flow or 
the cloud SDK calls
efficiently like Roots does. As a result they cannot pinpoint potential
performance bottlenecks in the underlying cloud platform.

Dautov, Paraskasis and Stannett showed that a cloud platform monitor
can be organized as a sensor network. They argue that cloud platforms are 
...
framework for the CloudFoundry PaaS. This solution is organized
...
performance anomalies in web applications. Some of their work
also addresses root cause analysis to some extent. They use
...
change point detection methods in the Roots implementation. 
...
have been studied by a number of researchers in the past. However,
...
difficult problem to solve. Roots is one step towards this direction,
and by making monitoring an integrated part of the cloud we believe
that we can deal with all the above challenges satisfactorily. 
