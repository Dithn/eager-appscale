...
operation on Google App Engine as a case study and analyze the results in more detail. Note that this is the case which exhibits the largest 
...
App Engine. This distribution was obtained by considering the benchmarking results gathered in a window of 1000 minutes. 
...
\item About 10\% of the values are higher than 4500ms; more than 1000ms larger than the mean
\end{itemize}

These two observations imply a significant skew away from the mean, and the presence of many high outliers.
Therefore it becomes evident that StudentInfo\#getAllStudents operation can result in very high execution times (i.e. high outliers) 
regularly.  In order to incorporate some of these high outliers in the distribution,
...
many high outliers, Cerebro predictions get less tight. In other words Cerebro is trading off tightness of the predictions for their accuracy.
...
To further reinforce this observation, we also take a close look at one of the web API operations that result in very tight predictions. 
...
This distribution has a mean of 9.19 and a median of 9. Given that only 19\% of the values are larger than the mean, and only 2\%
of the values are larger than 18ms (twice the mean), 
...
tighter predictions, without compromising the accuracy of the results.  
