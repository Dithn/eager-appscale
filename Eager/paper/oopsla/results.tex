We next describe our experimental evaluation of Cerebro.  
We conduct tests using five open source, App Engine web APIs.
\texttt{StudentInfo} REST-based (JAX-RS) application for managing students 
of a class (adding, removing, and listing student information).
\texttt{ServerHealth} monitors, computes, and reports statistics for server uptime
for a given web URL.
\texttt{SocialMapper} is as social networking application with. 
\texttt{StockTrader} is a stock trading application that provides APIs for adding 
users, registering companies, buying and selling stocks among users.
\texttt{Rooms} is a hotel booking application with APIs for registering hotels and querying
available rooms.
These web APIs use the cloud SDK datastore interface extensively. The rooms web API
also uses the memcache interface. We focus on these two interfaces exclusively in this study.
We execute these applications in the Google App Engine public cloud and in an AppScale
private cloud.  We instrument the programs to collect execution time statistics.
...
We first evaluate the correctness and accuracy of Cerebro predictions. 
We benchmark each web API 
for a period of 15 to 20 hours.
During this time we run a remote HTTP client that makes requests
to the web APIs once every minute.  The instrumented web APIs measure 
and record the execution time of the API operation for each request (server side).
Concurrently and within the same PaaS system, we execute the Cerebro Watchtower which
profiles cloud SDK operations independently every minute.
