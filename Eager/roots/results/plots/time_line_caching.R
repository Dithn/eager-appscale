ts_data <- read.table('input_time_line_caching.txt', sep=',')
times <- as.POSIXct(ts_data[,1], format='%Y-%m-%d %H:%M:%S')

dev.new(width=6, height=1.8)

start <- as.POSIXct('2016-08-29 01:00:00', format='%Y-%m-%d %H:%M:%S')
end <- as.POSIXct('2016-08-29 11:00:00', format='%Y-%m-%d %H:%M:%S')
