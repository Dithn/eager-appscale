ts_data <- read.table('input_time_line_guestbook_2s_datastore.txt', sep=',')
times <- as.POSIXct(ts_data[,1], format='%Y-%m-%d %H:%M:%S')

dev.new(width=6, height=1.8)

start <- as.POSIXct('2016-08-19 13:00:00', format='%Y-%m-%d %H:%M:%S')
end <- as.POSIXct('2016-08-19 23:00:00', format='%Y-%m-%d %H:%M:%S')
