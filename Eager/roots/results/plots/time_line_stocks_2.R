ts_data <- read.table('input_time_line_stocks_2.txt', sep=',')
times <- as.POSIXct(ts_data[,1], format='%Y-%m-%d %H:%M:%S')

dev.new(width=6, height=1.8)

start <- as.POSIXct('2016-08-31 03:00:00', format='%Y-%m-%d %H:%M:%S')
end <- as.POSIXct('2016-08-31 13:00:00', format='%Y-%m-%d %H:%M:%S')
...
for (hour in c(4, 6, 8, 10, 12)) {
    start2 <- as.POSIXct(paste('2016-08-31 ', hour, ':00:00', sep=''), format='%Y-%m-%d %H:%M:%S')
