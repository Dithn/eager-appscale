...
class ESReporter:
  def __init__(self, host, port, index='datastore', es_type='appscale'):
    self.host = host
    self.port = port
    self.path = "/{0}/{1}".format(index, es_type)
    self.events = []
    thread.start_new_thread(self.report_events, ())
    logging.info('ESReporter initialized')
    ...
    while True:
      try:
        if len(self.events) > 0:
          ...
          self.__report_to_es(event)
        else:
          time.sleep(1)
      except Exception, ex:
        logging.error('Error while reporting to ElasticSearch: {0}'.format(str(ex)))

  def __report_to_es(self, event):
    conn = httplib.HTTPConnection(self.host, self.port)
    conn.request('POST', self.path, json.dumps(event), {'Content-type': 'application/json'})
    response = conn.getresponse()
    logging.debug('Reported event to ElasticSearch: {0}, {1}'.format(response.status, response.read()))
    conn.close()
...
class MainHandler(tornado.web.RequestHandler):
  ...
  def remote_request(self, app_id, http_request_data):
    ...
    self.hkj_log(app_id, apirequest)
