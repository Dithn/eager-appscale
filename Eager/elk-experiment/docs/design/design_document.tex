...
we will take an approach which combines static analysis of the hosted web APIs and runtime monitoring of the PaaS cloud. 
...
static analysis to identify the PaaS kernel service invocations that dominate the response time of web APIs.
...
This component analyzes the source code of the web application and extracts a sequence of cloud SDK operations. It performs a simple
construction and interprocedural static analysis of control flow graph for each web application operation. Ideally, we can identify
the PaaS kernel service calls by their namespace.
...
This agent monitors and records the response time of individual PaaS services within a running PaaS system. It can be built as
...
web applications, the monitoring agent runs in the background separate from them.
The agent invokes services provided by PaaS kernel periodically and records response times for each PaaS service. Also, the agent periodically
