...
int main(int agrc, char** argv) {
  ...
  long bit_map_size_in_blocks = ceil(ceil(inode_count/sizeof(char))/BLOCK_SIZE); 
  ...
  int i;
  block_id current = 1;
  for (i = 0; i < bit_map_size_in_blocks; i++) {
  ...
  printf("Blocks available for data: %ld\n", remaining);
  ...
  printf("inode count: %ld, bit map size in blocks: %ld \n", inode_count, bit_map_size_in_blocks);
  printf("size of inode: %ld \n", sizeof(inode));
  printf("block_count: %ld, inode percentage: %f, total bytes: %ld\n", block_count, inode_percentage, total_num_bytes);
  close_device();
  return 0;
}
